...
describe MailBox do
  ...
  describe "Accessors" do
    ...
    it "should not set letters" do
      @mailbox1.letters= 2394 if defined? @mailbox1.letters=()
      expect(@mailbox1.letters).to eq(4)
    end
    it "should not set packages" do
      @mailbox1.packages= 2394 if defined? @mailbox1.packages=()
      expect(@mailbox1.packages).to eq(8)
    end
    it "should get the total items in all mailboxes" do
      expect(@mailbox1.class.total_items).to eq(39)
    end
    it "should get the total number of mailbox objects" do
      expect(@mailbox1.class.mailboxes).to eq(2)
    end
  end
